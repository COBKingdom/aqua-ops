...
function MovementCard({ label, value, unit, color }: { label: string; value: number; unit: string; color: string }) {
...
function DataEntryStockScreen({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [
        ...
        { data: rProd }, { data: rSales }, { data: rExp },
      ] = await Promise.all([
        supabase.from("production").select("product_type,bags_produced").eq("factory_id", factoryId),
        supabase.from("sales").select("product_type,bags_sold").eq("factory_id", factoryId),
        ...
        <div className="flex gap-2 shrink-0">
          <button
            ...
            className={`flex items-center gap-1 text-xs font-semibold px-3 py-2 rounded-xl transition ${
              filter === "today" ? "bg-[#2563eb] text-white shadow-sm" : "bg-white text-gray-500 border border-gray-200"
            }`}
          >
            📅 Today
          </button>
          <button
            ...
            className={`flex items-center gap-1 text-xs font-semibold px-3 py-2 rounded-xl transition ${
              filter === "7days" ? "bg-[#2563eb] text-white shadow-sm" : "bg-white text-gray-500 border border-gray-200"
            }`}
          >
            📅 Last 7 Days
          </button>
        ...
        <>
          ...
          <div>
            <p className="text-sm font-bold text-[#0d1b3e] mb-3">
              {filter === "today" ? "Today's Movement" : "Last 7 Days Movement"}
            </p>
            <div className="grid grid-cols-2 gap-3">
          ...
          {/* RECENT ACTIVITY */}
          <div>
            <p className="text-sm font-bold text-[#0d1b3e] mb-3">Recent Activity</p>
            <div className="flex gap-3 overflow-x-auto pb-2 -mx-1 px-1">

              {/* PRODUCTION */}
              <div className="min-w-[185px] bg-green-50 rounded-2xl p-3 flex flex-col shrink-0">
                ...
                <div className="space-y-2.5 flex-1">
                  {recentProd.length === 0 ? (
                    ...
                    <div key={r.id} className="flex items-center justify-between">
                      ...
                      <p className="text-xs font-bold text-green-700 ml-1">{r.bags_produced} bags</p>
                    </div>
                  ))}
                </div>
                {setActiveTab && (
                  <button onClick={() => setActiveTab("production")} className="flex items-center gap-1 text-[10px] text-green-700 font-semibold mt-3 pt-2 border-t border-green-100">
              ...
              {/* SALES */}
              <div className="min-w-[185px] bg-red-50 rounded-2xl p-3 flex flex-col shrink-0">
                <div className="flex items-center gap-2 mb-3">
                ...
                <div className="space-y-2.5 flex-1">
                  {recentSales.length === 0 ? (
                    ...
                    <div key={r.id} className="flex items-center justify-between">
                      ...
                      <p className="text-xs font-bold text-red-600 ml-1">{r.bags_sold} bags</p>
                    </div>
                  ))}
                </div>
                {setActiveTab && (
                  <button onClick={() => setActiveTab("sales")} className="flex items-center gap-1 text-[10px] text-red-600 font-semibold mt-3 pt-2 border-t border-red-100">
              ...
              {/* EXPENSES */}
              <div className="min-w-[185px] bg-purple-50 rounded-2xl p-3 flex flex-col shrink-0">
                <div className="flex items-center gap-2 mb-3">
                ...
                <div className="space-y-2.5 flex-1">
                  {recentExp.length === 0 ? (
                    ...
                    <div key={r.id} className="flex items-center justify-between">
                      ...
                      <p className="text-xs font-bold text-purple-700 ml-1">₦{Number(r.amount).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
                {setActiveTab && (
                  <button onClick={() => setActiveTab("expenses")} className="flex items-center gap-1 text-[10px] text-purple-700 font-semibold mt-3 pt-2 border-t border-purple-100">
...
export function StockScreen({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
